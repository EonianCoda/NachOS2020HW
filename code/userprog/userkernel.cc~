		...
		: ThreadedKernel(argc, argv)
{
	...
	memset(execfileBurstTime, 0, 10);
	memset(execfileArrivalTime, 0, 10);
    for (int i = 1; i < argc; i++) {
		...
		else if(strcmp(argv[i], "-arrive") == 0)
		{
...
UserProgKernel::Run()
{
	...
	for (int n=1;n<=execfileNum;n++)
	{
		...
		t[n]->remainingTime = execfileBurstTime[n];
		DEBUG(dbscheduler, execfile[n]);
		DEBUG(dbscheduler, execfilePriority[n]);
		
		t[n]->arrivalTime = kernel->stats->userTicks;
		t[n]->callbackArrivalTime = execfileArrivalTime[n];
		if(execfileArrivalTime[n] == 0)
		{
